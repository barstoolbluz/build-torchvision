# TorchVision with PyTorch 2.10.0 for CPU-only + AVX2
...
let
  ...
  customPytorch = (nixpkgs_pinned.python3Packages.torch.override {
    ...
    pname = "pytorch210-for-torchvision-cpu-avx2";
    patches = [];
    ninjaFlags = [ "-j32" ];
...
in
  (nixpkgs_pinned.python3Packages.torchvision.override {
    ...
    preConfigure = (oldAttrs.preConfigure or "") + ''
      ...
      export CFLAGS="${nixpkgs_pinned.lib.concatStringsSep " " cpuFlags} $CFLAGS"
      export MAX_JOBS=32
      echo "CPU-only build | CPU: AVX2 | PyTorch: 2.10.0"
    '';
    meta = oldAttrs.meta // {
      description = "TorchVision CPU-only + AVX2 with PyTorch 2.10.0";
      longDescription = ''
